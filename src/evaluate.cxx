// Market evaluator
// Platform-agnostic strategy evaluation that reads market data and generates
// signals Run this after fetch to analyse market snapshots and identify
...
int main() {
  ...
  for (const auto &candidate : candidates) {
    ...
    if (bars.size() < 20) {
      std::println("Warning: Insufficient bars for {} (got {})",
                   candidate.symbol, bars.size());
      continue;
    }
    ...
    if (should_enter) {
      ...
      std::println("ENTRY SIGNAL: {} using {} @ ${:.2f}", candidate.symbol,
                   candidate.strategy, bars.back().close);
    }
  }
