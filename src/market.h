...
constexpr bool is_market_close(std::string_view timestamp) {
...
constexpr bool is_liquidation_time(std::string_view timestamp) {
	auto hour = extract_hour(timestamp);
	auto minute = extract_minute(timestamp);
...
constexpr bool is_market_open(std::string_view timestamp) {
	auto hour = extract_hour(timestamp);
	auto minute = extract_minute(timestamp);
...
constexpr bool is_risk_on(std::string_view timestamp) {
	auto hour = extract_hour(timestamp);
	auto minute = extract_minute(timestamp);
...
namespace {
	...
	static_assert(is_market_open("2026-02-16T14:30:00Z"), "Market should be open at 14:30 UTC");
	static_assert(is_market_open("2026-02-16T15:00:00Z"), "Market should be open at 15:00 UTC");
	static_assert(is_market_open("2026-02-16T20:59:00Z"), "Market should be open at 20:59 UTC");

	// Test: market closed
	static_assert(!is_market_open("2026-02-16T14:29:00Z"), "Market should be closed at 14:29 UTC");
	static_assert(!is_market_open("2026-02-16T21:00:00Z"), "Market should be closed at 21:00 UTC");
	static_assert(!is_market_open("2026-02-16T21:01:00Z"), "Market should be closed at 21:01 UTC");

	// Test: market close detection
	static_assert(is_market_close("2026-02-16T21:00:00Z"), "Should detect close at 21:00 UTC");
	static_assert(is_market_close("2026-02-16T21:30:00Z"), "Should detect close after 21:00 UTC");
	static_assert(!is_market_close("2026-02-16T20:59:00Z"), "Should not detect close before 21:00 UTC");

	// Test: liquidation time (10 minutes before close = 20:50 UTC)
	static_assert(is_liquidation_time("2026-02-16T20:50:00Z"), "Should liquidate at 20:50 UTC");
	static_assert(is_liquidation_time("2026-02-16T20:55:00Z"), "Should liquidate at 20:55 UTC");
	static_assert(!is_liquidation_time("2026-02-16T20:49:00Z"), "Should not liquidate before 20:50 UTC");
	...
	static_assert(!is_risk_on("2026-02-16T14:30:00Z"), "Risk off at market open");
	static_assert(!is_risk_on("2026-02-16T15:29:00Z"), "Risk off before 1 hour");
	static_assert(is_risk_on("2026-02-16T15:30:00Z"), "Risk on at 15:30 UTC");
	static_assert(is_risk_on("2026-02-16T18:00:00Z"), "Risk on during mid-day");
	static_assert(is_risk_on("2026-02-16T20:29:00Z"), "Risk on before cutoff");
	static_assert(!is_risk_on("2026-02-16T20:30:00Z"), "Risk off at 30 min before close");
	static_assert(!is_risk_on("2026-02-16T20:45:00Z"), "Risk off near close");
}
