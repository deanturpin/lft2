...
static_assert(utc_offset(3)  == minutes{-5h});  // March     - EST (transition month, non-trading Sunday)
...
static_assert(utc_offset(11) == minutes{-5h});  // November  - EST (transition month, non-trading Sunday)
...
constexpr minutes ny_minutes(std::string_view ts) {
	...
	if (local_min < minutes{0})  local_min += day_min;
	if (local_min >= day_min)    local_min -= day_min;
	return local_min;
}
static_assert(ny_minutes("2026-02-16T14:30:00Z") == 9h + 30min);  // 14:30 UTC = 09:30 EST
static_assert(ny_minutes("2026-07-01T13:30:00Z") == 9h + 30min);  // 13:30 UTC = 09:30 EDT
static_assert(ny_minutes("2026-02-16T21:00:00Z") == 16h);          // 21:00 UTC = 16:00 EST
static_assert(ny_minutes("2026-07-01T20:00:00Z") == 16h);          // 20:00 UTC = 16:00 EDT
...
static_assert( risk_off("2026-02-16T15:29:00Z"));   // 10:29 ET - one minute before safe
static_assert(!risk_off("2026-02-16T15:30:00Z"));   // 10:30 ET - safe window starts
static_assert(!risk_off("2026-02-16T18:00:00Z"));   // 13:00 ET - mid-day
static_assert(!risk_off("2026-02-16T20:29:00Z"));   // 15:29 ET - one minute before cutoff
